...
export function useInventoryRequests(entityId?: string | null) {
    ...
    const fetchRequests = useCallback(
        async ({
            ...
            const normalized = (
                ...
                source_store_code:
                    row.source_store?.code ?? "",

                source_store_name:
                    row.source_store?.name ?? "",

                destination_store_code:
                    row.destination_store?.code ?? "",
    ...
    const fetchStoreItems = useCallback(
        async (storeId: string) => {
            ...
            for (
                const row of
                (data ?? []) as unknown as StoreItemQueryRow[]
            ) {
                const item = row.item;
    ...
    > => {
        ...
        return (
            (data ?? []) as InventoryRequestItemRow[]
        ).map((row) => ({
            id: row.id,
    ...
    const create = async (
        payload: InventoryRequestFormData
    ) => {
        setSaving(true);
        ...
        if (!data?.success) {
            ...
            setError(
                data?.message ??
                    "Gagal membuat Inventory Request"
            );
