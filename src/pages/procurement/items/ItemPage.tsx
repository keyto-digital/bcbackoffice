...
export default function ItemPage() {
  ...
  useEffect(() => {
    ...
    const editableName =
      editingItem.name.startsWith(
        savedPrefix
      )
        ? editingItem.name.slice(
            savedPrefix.length
          )
        : editingItem.name;
    ...
    setFormData({
      ...
      inventory_account_id:
        editingItem.inventory_account_id ??
        "",

      expense_account_id:
        editingItem.expense_account_id ??
        "",

      cogs_account_id:
        editingItem.cogs_account_id ??
        "",
  ...
  const handleSubmit =
    async (
      ...
      if (
        ...
        window.alert(
          "Kode dan nama artikel wajib diisi."
        );
      ...
      if (
        ...
        window.alert(
          "Harga standar tidak boleh negatif."
        );
  ...
  const exportExcel = async () => {
    try {
      if (totalCount === 0) {
        window.alert(
          "Tidak ada data artikel yang dapat diexport."
        );
        return;
      }

      const exportData =
        await exportItems(search);
    ...
    <div className="w-full pr-2 space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Hubungkan artikel dengan kategori, COA,
          satuan, dan perlakuan stok.
        </p>
      </div>
      ...
      {loadingMasters && (
        <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
          Memuat kategori, subkategori, satuan,
          dan akun COA...
        </div>
      )}
      ...
      >
        ...
        <div className="mb-5 flex items-center justify-between">
          ...
          <div>
            ...
            <h2 className="text-base font-semibold text-gray-900">
              {editingItem
                ? "Edit Artikel Barang"
                : "Tambah Artikel Barang"}
            </h2>

            <p className="mt-1 text-sm text-gray-500">
              Kosongkan akun override agar item mengikuti
              akun kategori atau subkategorinya.
            </p>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={
                formData.stock_adjustment_account_id
              }
              onChange={(
                event
              ) =>
